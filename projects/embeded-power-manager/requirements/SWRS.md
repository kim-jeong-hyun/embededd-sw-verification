...
#### REQ-EPM-002
- Wake up Signal이 감지되거나 CAN Wake up Request를 수신하면 본 시스템은 POWER_OFF 상태에서 INIT 상태로 전환해야 한다. 

#### REQ-EPM-003
- INIT 상태에서 초기화가 정상적으로 완료되면 ACTIVE 상태로 전환해야 한다.

#### REQ-EPM-004
- Battery Voltage가 10V 이상 11.5V 미만이면면 LOW POWER 상태로 전환해야 한다.
...
#### REQ-EPM-007
- Sleep Timer가 30초 경과하면 SLEEP 상태로 전환해야 한다.
...
#### REQ-EPM-011
- ACTIVE 상태에서는 수신된 CAN Mesage를 처리해야 한다.
...
#### REQ-EPM-014
- Fault Reset 명령을 수신하고 Fault 조건이 해제되면 INIT 상태로 전환해야 한다.
